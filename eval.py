...
from evaluation.visual_ogbench_envs import register_visual_ogbench_envs
...
TASK_ALIASES = {
    ...
    "reacher": "reacher",
    "researcher": "reacher",
    "visual-puzzle-3x3": "visual_puzzle_3x3",
    "visual_puzzle_3x3": "visual_puzzle_3x3",
    "visual-antmaze-large": "visual_antmaze_large",
    "visual_antmaze_large": "visual_antmaze_large",
}
...
class PlanningStatsSolver:
    ...
    def configure(self, *args: Any, **kwargs: Any) -> Any:
        result = self.solver.configure(*args, **kwargs)
        self._expand_flat_action_bounds()
        return result

    def _expand_flat_action_bounds(self) -> None:
        """Match iCEM/Gradient bounds to flattened action-block candidates."""
        if type(self.solver).__name__ not in {"ICEMSolver", "GradientSolver"}:
            return
        action_low = getattr(self.solver, "_action_low", None)
        action_high = getattr(self.solver, "_action_high", None)
        action_dim = getattr(self.solver, "action_dim", None)
        if action_low is None or action_high is None or action_dim is None:
            return

        flat_size = int(action_low.numel() if torch.is_tensor(action_low) else np.size(action_low))
        target_size = int(action_dim)
        if flat_size == target_size or flat_size <= 0 or target_size % flat_size != 0:
            return

        repeat_factor = target_size // flat_size
        if torch.is_tensor(action_low):
            self.solver._action_low = action_low.reshape(-1).repeat(repeat_factor)
            self.solver._action_high = action_high.reshape(-1).repeat(repeat_factor)
        else:
            self.solver._action_low = np.tile(np.asarray(action_low).reshape(-1), repeat_factor)
            self.solver._action_high = np.tile(np.asarray(action_high).reshape(-1), repeat_factor)
        print(
            "[solver-adapter] "
            f"solver={type(self.solver).__name__} expanded_action_bounds="
            f"{flat_size}->{target_size}"
        )
...
def resolve_diffusion_sampling_config(cfg: DictConfig) -> dict[str, float]:
    """Resolve optional diffusion scalars while preserving explicit zero values."""
    return {
        "eta": float(value_or_default(cfg.get("diffusion_eta", None), 0.0)),
        "noise_scale": float(
            value_or_default(cfg.get("diffusion_noise_scale", None), 1.0)
        ),
        "sampling_temperature": float(
            value_or_default(cfg.get("diffusion_sampling_temperature", None), 1.0)
        ),
    }
...
EVALUATION_PROTOCOL_ALIASES = {
    "dataset": "dataset_future",
    "dataset_future": "dataset_future",
    "future_goal": "dataset_future",
    "official": "official_tasks",
    "official_tasks": "official_tasks",
    "ogbench": "official_tasks",
}


def resolve_evaluation_protocol(cfg: DictConfig) -> str:
    raw_protocol = str(cfg.eval.get("protocol", "dataset_future")).strip().lower()
    protocol = EVALUATION_PROTOCOL_ALIASES.get(raw_protocol)
    if protocol is None:
        raise ValueError(
            f"Unknown eval.protocol '{raw_protocol}'. Expected one of: "
            "dataset_future, official_tasks."
        )
    return protocol


def resolve_official_task_ids(cfg: DictConfig) -> list[int]:
    raw_task_ids = OmegaConf.to_container(cfg.eval.get("task_ids"), resolve=True)
    if not isinstance(raw_task_ids, list) or not raw_task_ids:
        raise ValueError(
            "eval.task_ids must be a non-empty list when eval.protocol=official_tasks."
        )
    task_ids = [int(task_id) for task_id in raw_task_ids]
    if any(task_id <= 0 for task_id in task_ids):
        raise ValueError(f"Official task IDs must be positive, got {task_ids}.")
    if len(set(task_ids)) != len(task_ids):
        raise ValueError(f"Official task IDs must be unique, got {task_ids}.")
    return task_ids


def _reset_policy_between_official_batches(policy: Any) -> None:
    reset = getattr(policy, "reset", None)
    if reset is None:
        return
    try:
        reset(reset_stats=False)
    except TypeError:
        reset()


def _validate_official_reset_observations(world: Any, task_id: int) -> None:
    missing = [key for key in ("pixels", "goal") if key not in world.infos]
    if missing:
        raise KeyError(
            f"Official task {task_id} reset did not provide {missing}. "
            "The benchmark requires both current pixels and goal pixels."
        )
    pixels = _to_numpy(world.infos["pixels"])
    goal = _to_numpy(world.infos["goal"])
    if pixels.shape[0] != int(world.num_envs) or goal.shape[0] != int(world.num_envs):
        raise ValueError(
            "Official reset observations must be batched by world.num_envs: "
            f"pixels={pixels.shape}, goal={goal.shape}, num_envs={world.num_envs}."
        )
    if pixels.shape != goal.shape:
        raise ValueError(
            "Current and goal observations must have matching shapes after World "
            f"preprocessing, got pixels={pixels.shape}, goal={goal.shape}."
        )


def run_official_task_evaluation(
    *,
    world: Any,
    policy: Any,
    task_ids: list[int],
    ...
    """Evaluate fixed OGBench tasks using reset-provided current and goal images."""
    ...
    if int(world.num_envs) <= 0:
        raise ValueError(f"world.num_envs must be positive, got {world.num_envs}.")
    if not task_ids:
        raise ValueError("task_ids must be non-empty for official task evaluation.")

    task_success: dict[str, float] = {}
    all_successes: list[np.ndarray] = []
    all_task_ids: list[np.ndarray] = []
    all_seeds: list[np.ndarray] = []
    batch_size = int(world.num_envs)

    for task_position, task_id in enumerate(task_ids):
        task_episode_successes: list[np.ndarray] = []
        for batch_start in range(0, episodes_per_task, batch_size):
            active_count = min(batch_size, episodes_per_task - batch_start)
            batch_seed_base = int(seed) + task_position * episodes_per_task
            seeds = np.arange(
                batch_seed_base + batch_start,
                batch_seed_base + batch_start + batch_size,
                dtype=np.int64,
            )
            if task_position > 0 or batch_start > 0:
                _reset_policy_between_official_batches(policy)
            world.reset(
                seed=seeds.tolist(),
                options={"task_id": int(task_id)},
            )
            _validate_official_reset_observations(world, task_id)

            episode_successes = np.zeros(batch_size, dtype=bool)
            for _ in range(eval_budget):
                world.step()
                terminated = np.asarray(world.terminateds, dtype=bool).reshape(batch_size, -1).any(axis=1)
                info_success = world.infos.get("success", terminated)
                info_success = np.asarray(info_success, dtype=bool).reshape(batch_size, -1).any(axis=1)
                episode_successes |= terminated | info_success
                world.envs.unwrapped._autoreset_envs = np.zeros(batch_size, dtype=bool)
                if bool(np.all(episode_successes)):
                    break

            active_successes = episode_successes[:active_count].copy()
            task_episode_successes.append(active_successes)
            all_successes.append(active_successes)
            all_task_ids.append(np.full(active_count, task_id, dtype=np.int64))
            all_seeds.append(seeds[:active_count].copy())

        task_successes = np.concatenate(task_episode_successes)
        task_rate = float(np.mean(task_successes))
        task_name = f"task{task_id}"
        task_success[task_name] = task_rate
        print(
            "[official-task] "
            f"task_id={task_id} episodes={episodes_per_task} "
            f"success={task_rate:.6f} success_rate={100.0 * task_rate:.4f}"
        )

    episode_successes = np.concatenate(all_successes)
    overall_success = float(np.mean(list(task_success.values())))
    return {
        "evaluation_protocol": "official_tasks",
        "task_success": task_success,
        "overall_success": overall_success,
        "success_rate": 100.0 * overall_success,
        "episode_successes": episode_successes,
        "episode_task_ids": np.concatenate(all_task_ids),
...
class PixelResizedEvaluationDataset:
    """Resize pixel sequences loaded for evaluation while proxying dataset metadata."""

    def __init__(self, dataset: Any, image_size: int) -> None:
        if int(image_size) <= 0:
    ...
    def _resize_pixels(self, value: Any) -> Any:
        ...
        if tensor.ndim != 4 or tensor.shape[1] not in (1, 3, 4):
            raise ValueError(
                "Evaluation pixel sequences must have TCHW layout, "
        ...
        if not dtype.is_floating_point:
            ...
            if dtype == torch.uint8:
                resized = resized.clamp(0, 255)
        resized = resized.to(dtype=dtype)
        return resized if is_tensor else resized.cpu().numpy()
    ...
    def load_chunk(self, episodes_idx, start, end) -> list[dict]:
        ...
        for episode in chunks:
            for key, value in list(episode.items()):
                if key.startswith("pixels") and isinstance(value, (torch.Tensor, np.ndarray)):
                    episode[key] = self._resize_pixels(value)
        return chunks
...
def resolve_plain_eval_video_options(
    results_path: Path,
    trajectory_quality_cfg: dict,
) -> dict[str, Any]:
    return {
        "save_video": bool(trajectory_quality_cfg.get("save_video", True)),
        "video_path": results_path,
    }


def resolve_plain_eval_video_path(
    results_path: Path,
    trajectory_quality_cfg: dict,
) -> Path:
    return resolve_plain_eval_video_options(results_path, trajectory_quality_cfg)["video_path"]
...
def _pixel_frames_to_hwc(value) -> np.ndarray:
    """Normalize pixel frames to [..., height, width, channels]."""
    array = _to_numpy(value)
    if array.ndim < 3:
        raise ValueError(f"Pixel frames must have at least 3 dimensions, got {array.shape}.")
    if int(array.shape[-1]) in {1, 3, 4}:
        return array
    if int(array.shape[-3]) in {1, 3, 4}:
        return np.moveaxis(array, -3, -1)
    raise ValueError(
        "Could not identify the pixel channel axis in frame array "
...
def _trajectory_state_keys_for_task(task: str | None) -> list[str]:
    ...
    if task_name == "visual_antmaze_large":
        return [*common, "qpos", "qvel"]
...
def run_evaluation_with_trajectory_quality(
    *,
    ...
    target_frames = np.stack([_pixel_frames_to_hwc(ep["pixels"]) for ep in data])
...
def sample_eval_episode_starts(
    *,
    ...
    if valid_start_count == 0:
        raise ValueError("No valid starting points found for dataset-future evaluation.")

    filtered_episode_positions = None
    filtered_start_steps = None
    filtered_global_rows = None
    if pair_filter is not None:
        filter_key = str(pair_filter["key"])
        min_changed_elements = int(pair_filter.get("min_changed_elements", 1))
        if min_changed_elements <= 0:
            raise ValueError(
                "eval.dataset_future_filter.min_changed_elements must be positive, "
                f"got {min_changed_elements}."
            )

        start_counts = max_start[valid_episode_positions] + 1
        candidate_episode_positions = np.repeat(valid_episode_positions, start_counts)
        candidate_start_steps = np.concatenate(
            [np.arange(int(count), dtype=np.int64) for count in start_counts]
        )
        candidate_global_rows = (
            offsets[candidate_episode_positions] + candidate_start_steps
        )
        candidate_goal_rows = candidate_global_rows + int(goal_offset_steps) - 1

        filter_values = np.asarray(dataset.get_col_data(filter_key))
        if filter_values.ndim == 0 or int(filter_values.shape[0]) <= int(candidate_goal_rows.max()):
            raise ValueError(
                f"Dataset filter key '{filter_key}' does not cover sampled goal rows: "
                f"shape={filter_values.shape}, max_goal_row={int(candidate_goal_rows.max())}."
            )
        start_values = filter_values[candidate_global_rows].reshape(valid_start_count, -1)
        goal_values = filter_values[candidate_goal_rows].reshape(valid_start_count, -1)
        changed_elements = np.count_nonzero(start_values != goal_values, axis=1)
        keep = changed_elements >= min_changed_elements
        filtered_episode_positions = candidate_episode_positions[keep]
        filtered_start_steps = candidate_start_steps[keep]
        filtered_global_rows = candidate_global_rows[keep]
        valid_start_count = int(np.count_nonzero(keep))
    ...
    if filtered_episode_positions is not None:
        choices = np.sort(
            rng.choice(valid_start_count, size=int(num_eval), replace=False)
        )
        episode_positions = filtered_episode_positions[choices]
        start_steps = filtered_start_steps[choices]
        global_rows = filtered_global_rows[choices]
...
def get_dataset(cfg, dataset_name):
    ...
    eval_image_size = cfg.dataset.get("eval_image_size", None)
    if eval_image_size not in [None, "", "null"]:
        dataset = PixelResizedEvaluationDataset(dataset, int(eval_image_size))
    return dataset


def resolve_dataset_future_pair_filter(cfg: DictConfig) -> dict[str, Any] | None:
    filter_cfg = cfg.eval.get("dataset_future_filter", None)
    if filter_cfg is None:
        return None
    raw_filter = OmegaConf.to_container(filter_cfg, resolve=True)
    if not isinstance(raw_filter, dict):
        raise TypeError(
            "eval.dataset_future_filter must resolve to a dictionary, "
            f"got {type(raw_filter).__name__}."
        )
    filter_key = str(raw_filter.get("key", "")).strip()
    if not filter_key:
        raise ValueError("eval.dataset_future_filter.key must be non-empty.")
    min_changed_elements = int(raw_filter.get("min_changed_elements", 1))
    if min_changed_elements <= 0:
        raise ValueError(
            "eval.dataset_future_filter.min_changed_elements must be positive, "
            f"got {min_changed_elements}."
    ...
    return {
        "key": filter_key,
        "min_changed_elements": min_changed_elements,
    }
...
def run(cfg: DictConfig):
    ...
    evaluation_protocol = resolve_evaluation_protocol(cfg)
    official_task_ids = (
        resolve_official_task_ids(cfg) if evaluation_protocol == "official_tasks" else []
    )
    assert (
    ...
    cfg.world.num_envs = int(
        cfg.eval.get("parallel_envs", cfg.world.num_envs)
        if evaluation_protocol == "official_tasks"
        else cfg.eval.num_eval
    )
    cfg.world.max_episode_steps = (
        int(cfg.eval.eval_budget)
        if evaluation_protocol == "official_tasks"
        else 2 * int(cfg.eval.eval_budget)
    )
    ...
    ep_indices = None
    if evaluation_protocol == "dataset_future":
        ...
        infer_dataset_column(
            available_columns=available_columns,
            field_name="step_key",
            exact_candidates=["step_idx"],
        )
        ep_indices, _ = np.unique(
            stats_dataset.get_col_data(episode_key), return_index=True
        )
    ...
    diffusion_sampling_cfg = resolve_diffusion_sampling_config(cfg)
    diffusion_eta = diffusion_sampling_cfg["eta"]
    diffusion_noise_scale = diffusion_sampling_cfg["noise_scale"]
    diffusion_sampling_temperature = diffusion_sampling_cfg["sampling_temperature"]
    ...
    dataset_future_pair_filter = (
        resolve_dataset_future_pair_filter(cfg)
        if evaluation_protocol == "dataset_future"
        else None
    )
    if evaluation_protocol == "dataset_future" and len(eval_callables) == 0:
        print(
            "[metric-warning] "
    ...
    configured_results_dir = cfg.output.get("dir", None)
    if configured_results_dir not in [None, "", "null"]:
        results_path = Path(str(configured_results_dir)).expanduser()
    else:
        results_path = (
    ...
    if evaluation_protocol == "official_tasks":
        print(
            "[eval] "
            f"protocol=official_tasks planner_type={planner_type} "
            f"task={requested_task or config_name} config={config_name} "
            f"policy={policy_name} task_ids={official_task_ids} "
            f"episodes_per_task={int(cfg.eval.num_eval)} "
            f"parallel_envs={int(world.num_envs)} eval_budget={int(cfg.eval.eval_budget)}"
        )
    else:
        print(
            "[eval] "
            f"protocol=dataset_future planner_type={planner_type} "
            f"task={requested_task or config_name} config={config_name} "
            f"policy={policy_name} dataset={cfg.eval.dataset_name} "
    ...
    if evaluation_protocol == "official_tasks":
        if bool(trajectory_quality_cfg["enabled"]):
            raise ValueError(
                "trajectory_quality.enabled is not supported by the official task "
                "protocol; use eval.protocol=dataset_future for trajectory diagnostics."
            )
        metrics = run_official_task_evaluation(
            world=world,
            policy=policy,
            task_ids=official_task_ids,
            episodes_per_task=int(cfg.eval.num_eval),
            eval_budget=int(cfg.eval.eval_budget),
            seed=int(cfg.seed),
        )
    elif bool(trajectory_quality_cfg["enabled"]):
        assert ep_indices is not None
        eval_episodes, eval_start_idx, valid_start_count = sample_eval_episode_starts(
            ...
            seed=int(cfg.seed),
            pair_filter=dataset_future_pair_filter,
        )
        ...
        metrics = world.evaluate_from_dataset(
            ...
            episodes_idx=eval_episodes.tolist(),
            callables=eval_callables,
            **resolve_plain_eval_video_options(results_path, trajectory_quality_cfg),
        )
    end_time = time.time()
    ...
    if isinstance(metrics, dict) and "task_success" in metrics:
        for task_name, task_rate in metrics["task_success"].items():
            print(f"[summary] {task_name}_success={float(task_rate):.6f}")
    ...
    with results_path.open("a") as f:
        ...
        f.write(f"evaluation_protocol: {evaluation_protocol}\n")
        if evaluation_protocol == "official_tasks":
            f.write(f"official_task_ids: {official_task_ids}\n")
            f.write(f"episodes_per_task: {int(cfg.eval.num_eval)}\n")
            f.write(f"parallel_envs: {int(world.num_envs)}\n")
            f.write(f"overall_success: {float(metrics['overall_success'])}\n")
            for task_name, task_rate in metrics["task_success"].items():
                f.write(f"{task_name}_success: {float(task_rate)}\n")
        if planner_type == "mpc":
            solver_target, solver_params = summarize_solver_config(cfg.solver)
