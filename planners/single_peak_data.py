...
def encode_current_goal(
    ...
    z_goal: [embed_dim]
    """
    z_cur, z_goal = encode_current_goal_batch(
        model=model,
        info_dict={
            "pixels": info_dict["pixels"][env_index : env_index + 1],
            "goal": info_dict["goal"][env_index : env_index + 1],
        },
    )
    return z_cur[0], z_goal[0]
...
def encode_current_goal_batch(
    ...
    """Encode a batch of current and goal observations in two model calls."""
    ...
    encoded_current = _encode_pixels_only(model=model, pixels=info_dict["pixels"])
    encoded_goal = _encode_pixels_only(model=model, pixels=info_dict["goal"])
    z_cur = encoded_current["emb"][:, -1].detach().cpu().float()
    z_goal = encoded_goal["emb"][:, -1].detach().cpu().float()
    if z_cur.ndim != 2 or z_goal.ndim != 2 or z_cur.shape != z_goal.shape:
        raise ValueError(
            "Batched z_cur and z_goal must have matching [batch, embed_dim] shapes, "
            f"got {tuple(z_cur.shape)} and {tuple(z_goal.shape)}."
        )
    return z_cur, z_goal
