...
class SinglePeakPolicy(BasePolicy):
    ...
    def get_action(self, info_dict: dict[str, Any], **kwargs: Any) -> np.ndarray:
        ...
        if len(self._action_buffer) == 0:
            planner_device = next(self.planner.parameters()).device
            if planner_device.type == "cuda":
                torch.cuda.synchronize(planner_device)
            planning_started_at = time.perf_counter()
            plan = self.plan_actions(prepared_info)  # [num_envs, plan_horizon, action_dim]
            if planner_device.type == "cuda":
                torch.cuda.synchronize(planner_device)
            self._planning_time_total_sec += time.perf_counter() - planning_started_at
