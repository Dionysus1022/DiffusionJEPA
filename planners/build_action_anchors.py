...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build reusable anchors by fitting K-means and selecting the nearest "
            "real action chunk for every centroid."
        ),
    )
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        "--selection",
        choices=["kmeans-nearest"],
        default="kmeans-nearest",
        help="Anchor selection rule. Submission runs always use nearest real chunks.",
    )
    parser.add_argument(
        "--seed",
        type=int,
...
def main() -> None:
    ...
    local_anchor_indices = torch.tensor(
        anchor_bundle.metadata["selected_teacher_plan_indices"],
        dtype=torch.long,
    )
    anchor_bundle.metadata["selected_dataset_indices"] = indices.index_select(
        0, local_anchor_indices
    ).tolist()
