...
def load_goal_conditioned_row_batch(
    dataset,
    task_spec: TaskSpec,
    ...
    """Load trajectory supervision with one vectorized read per state column."""
    rows = np.asarray(row_indices, dtype=np.int64).reshape(-1)
    if rows.size == 0:
        raise ValueError("row_indices must be non-empty.")
    if goal_offset_steps <= 0 or context_steps <= 0:
        raise ValueError("goal_offset_steps and context_steps must be positive.")

    goal_rows = rows + int(goal_offset_steps) - 1
    endpoint_rows = np.concatenate([rows, goal_rows])
    unique_rows, inverse = np.unique(endpoint_rows, return_inverse=True)
    batch_size = int(rows.shape[0])
    current_positions = torch.from_numpy(inverse[:batch_size])
    goal_positions = torch.from_numpy(inverse[batch_size:])
    info_dict: dict[str, torch.Tensor] = {}

    for col in dataset._keys:
        resolved_key = "pixels" if col == task_spec.pixels_key else col
        if col == task_spec.action_key:
            action_data = np.asarray(dataset.get_col_data(col))
            action_rows = rows[:, None] + np.arange(int(context_steps), dtype=np.int64)[None, :]
            action_sequence = torch.from_numpy(np.asarray(action_data[action_rows]))
            goal_action = torch.from_numpy(np.asarray(action_data[goal_rows]))
            info_dict["action"] = action_sequence
            info_dict["goal_action"] = goal_action.unsqueeze(1)
            continue

        if col in dataset._cache:
            source = dataset._cache[col]
        else:
            dataset._open()
            source = dataset.h5_file[col]
        dense_start = int(unique_rows[0])
        dense_end = int(unique_rows[-1]) + 1
        dense_span = dense_end - dense_start
        if dense_span <= int(unique_rows.size) * 8:
            dense_values = np.asarray(source[dense_start:dense_end])
            values = dense_values[unique_rows - dense_start]
        else:
            values = np.asarray(source[unique_rows])
        if values.dtype == np.object_ or values.dtype.kind in ("S", "U"):
            continue
        tensor = torch.from_numpy(values)
        current_value = tensor[current_positions]
        goal_value = tensor[goal_positions]
        info_dict[resolved_key] = current_value.unsqueeze(1)
        goal_key = "goal" if resolved_key == "pixels" else f"goal_{resolved_key}"
        info_dict[goal_key] = goal_value.unsqueeze(1)
...
def main() -> None:
    ...
    for batch_start in range(0, args.num_samples, args.batch_size):
        ...
        try:
            if args.label_source == "trajectory":
                raw_info = load_goal_conditioned_row_batch(
                    dataset=dataset,
                    task_spec=task_spec,
            ...
            batch_z_cur, batch_z_goal = encode_current_goal_batch(
                model=wm_model,
        ...
        for env_index in range(current_bs):
            try:
                z_cur = batch_z_cur[env_index].clone()
                z_goal = batch_z_goal[env_index].clone()
                if args.label_source == "teacher":
