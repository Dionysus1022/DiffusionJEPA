...
class DiffusionPlannerPolicy(BasePolicy):
    """Runtime wrapper for the anchor-conditioned diffusion planner.

    Runtime flow:
        1. Preprocess raw env info via BasePolicy._prepare_info.
        2. Encode current obs and goal obs with the world model encoder.
        3. Run the configured reverse process from anchors to generate K candidate action chunks.
    ...
    def reset(self, *, reset_stats: bool = True) -> None:
        """Clear runtime state, optionally preserving cumulative planner statistics."""
        ...
        self._current_plan_index = 0
        self._current_plan_executed_steps = 0
        if reset_stats or not hasattr(self, "_num_replans"):
            self._num_replans = 0
            self._planning_time_total_sec = 0.0
            self._generation_time_total_sec = 0.0
            self._scoring_time_total_sec = 0.0
    ...
    def plan_actions(self, prepared_info: dict[str, torch.Tensor]) -> torch.Tensor:
        """Generate K candidates, select a winner, and reshape it into a plan.
        ...
        if self.selection_mode == "score_only":
            # Score-only ablates every rollout-based operation. The score logits
            # are produced during candidate generation, so selection is just Top-1.
            refined_candidates = candidates
            world_model_costs = torch.full_like(model_scores, float("inf"))
            refinement_time = 0.0
            scoring_time = 0.0
        ...
        elif self.selection_mode == "score_topk_wm":
            refined_candidates = candidates
    ...
    def generate_candidates(
        ...
        """Run the configured diffusion process to produce candidate action chunks.
    ...
    def score_and_select_candidates(
        ...
        if self.selection_mode == "score_only":
            # Score-only is an ablation of world-model verification, so it must
            # not pay for or depend on a latent rollout that it subsequently ignores.
            world_model_costs = torch.full_like(model_scores, float("inf"))
    ...
    def log_rerank_summary(
        ...
        *,
        ...
        print(
            ...
            f"diffusion_mode={self.planner.diffusion_mode} "
            f"initialization={self.planner.initialization_mode} "
