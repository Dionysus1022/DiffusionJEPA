...
class DiffusionPlannerModelConfig:
    """Hyperparameters for the anchor-conditioned diffusion planner."""

    latent_dim: int
    plan_horizon: int
    action_dim: int
    num_anchors: int
    initialization_mode: str = "anchors"
    diffusion_mode: str = "truncated"
    bridge_beta_d: float = 2.0
    bridge_beta_min: float = 0.1
    hidden_dim: int = 512
    num_layers: int = 3
    dropout: float = 0.0
    activation: str = "gelu"
...
def _normalize_model_hyperparameters(raw_hparams: dict[str, Any]) -> dict[str, Any]:
    ...
    hparams.setdefault("initialization_mode", "anchors")
    hparams.setdefault("diffusion_mode", "truncated")
    hparams.setdefault("bridge_beta_d", 2.0)
    hparams.setdefault("bridge_beta_min", 0.1)
    return hparams
...
def infer_model_config_from_dataset_and_anchor_bundle(
    ...
    *,
    ...
    return DiffusionPlannerModelConfig(
        ...
        bridge_beta_d=float(bridge_beta_d),
        bridge_beta_min=float(bridge_beta_min),
...
class DiffusionPlannerModel(nn.Module):
    """Anchor-conditioned planner with truncated and diffusion-bridge modes.
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.config = config
        self.config.initialization_mode = str(config.initialization_mode).lower().strip()
        if self.config.initialization_mode not in {"anchors", "gaussian"}:
            raise ValueError(
                "initialization_mode must be one of {'anchors', 'gaussian'}, "
                f"got {config.initialization_mode!r}."
            )
        self.config.diffusion_mode = str(config.diffusion_mode).lower().strip()
        if self.config.diffusion_mode not in {"truncated", "bridge"}:
            raise ValueError(
                "diffusion_mode must be one of {'truncated', 'bridge'}, "
                f"got {config.diffusion_mode!r}."
            )
        if self.config.diffusion_mode == "bridge" and self.config.initialization_mode != "anchors":
            raise ValueError("bridge diffusion requires initialization_mode='anchors'.")
        if (
            self.config.diffusion_mode == "bridge"
            and self.config.start_timestep is not None
            and int(self.config.start_timestep) != int(self.config.num_train_steps - 1)
        ):
            raise ValueError(
                "bridge diffusion start_timestep must be num_train_steps - 1 so inference starts from the anchor endpoint."
            )
        if self.config.diffusion_mode == "bridge":
            DiffusionBridgeSchedule(
                num_train_steps=int(self.config.num_train_steps),
                beta_d=float(self.config.bridge_beta_d),
                beta_min=float(self.config.bridge_beta_min),
            )
            if int(self.config.truncation_steps) > int(self.config.num_train_steps - 1):
                raise ValueError(
                    "bridge truncation_steps cannot exceed num_train_steps - 1 because "
                    "each denoiser evaluation must start at a positive timestep."
                )
    ...
    def initialization_mode(self) -> str:
        return str(self.config.initialization_mode)

    @property
    def diffusion_mode(self) -> str:
        return str(self.config.diffusion_mode)
    ...
    @property
    def bridge_schedule(self) -> DiffusionBridgeSchedule:
        return DiffusionBridgeSchedule(
    ...
    def resolve_inference_truncation_timesteps(
        ...
        *,
        ...
        returns:
            truncation_timesteps: descending state-time nodes. Truncated diffusion
                returns `truncation_steps` nodes; bridge diffusion returns
                `truncation_steps + 1` nodes for exactly `truncation_steps` denoiser
                evaluations ending at x_0.
        """
        default_timesteps = self.schedule.truncation_timesteps
        if (
            self.diffusion_mode != "bridge"
            and truncation_steps is None
        ...
        if self.diffusion_mode == "bridge" and runtime_start_timestep != self.bridge_schedule.terminal_timestep:
            raise ValueError(
                "Bridge inference must start from the terminal anchor timestep "
                f"{self.bridge_schedule.terminal_timestep}, got {runtime_start_timestep}."
            )
        state_node_count = (
            runtime_truncation_steps + 1
            if self.diffusion_mode == "bridge"
            else runtime_truncation_steps
        )
        return build_truncation_timesteps(
            num_train_steps=self.num_train_steps,
            truncation_steps=state_node_count,
            start_timestep=runtime_start_timestep,
            device=device,
        )  # [T_runtime]

    def build_inference_step_pairs(
        self,
        truncation_timesteps: torch.Tensor,
    ) -> list[tuple[int, int]]:
        """Pair state-time nodes without evaluating the bridge denoiser at t=0."""
        if truncation_timesteps.ndim != 1 or truncation_timesteps.numel() == 0:
            raise ValueError("truncation_timesteps must be a non-empty 1D tensor.")
        timestep_values = truncation_timesteps.detach().cpu().tolist()
        step_pairs = [
            (int(timestep_values[idx]), int(timestep_values[idx + 1]))
            for idx in range(len(timestep_values) - 1)
        ]
        if self.diffusion_mode == "bridge":
            if int(timestep_values[-1]) != 0:
                raise ValueError("Bridge inference timestep nodes must terminate at t=0.")
            return step_pairs
    ...
    def initialize_noisy_candidates(
        ...
        *,
        ...
        clean_candidates: torch.Tensor | None = None,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """Initialize candidates from noisy anchors or fresh Gaussian samples.
        ...
        if self.diffusion_mode == "bridge":
            if clean_candidates is None:
                if torch.any(timestep_grid != self.bridge_schedule.terminal_timestep):
                    raise ValueError(
                        "Bridge inference can only initialize from the terminal anchor timestep."
                    )
                return anchors, timestep_grid
            ...
            if clean.ndim == 2:
                if int(clean.shape[0]) != int(batch_size):
                    raise ValueError(
                        f"clean_candidates batch size {clean.shape[0]} does not match {batch_size}."
                    )
                clean = clean.unsqueeze(1).expand(-1, self.num_anchors, -1)
            elif clean.ndim == 3:
                expected_shape = (int(batch_size), self.num_anchors, self.action_chunk_dim)
                if tuple(clean.shape) != expected_shape:
                    raise ValueError(
                        f"clean_candidates must have shape {expected_shape}, got {tuple(clean.shape)}."
                    )
            else:
                raise ValueError(
                    "clean_candidates must have shape [B,D] or [B,K,D] for bridge training."
                )
            return (
                add_noise_to_bridge(
                    clean,
        ...
        if self.initialization_mode == "gaussian":
            if noise is None:
                noise = torch.randn(
                    int(batch_size),
                    self.num_anchors,
            ...
            if tuple(noise.shape) != expected_shape:
                raise ValueError(
                    "Fresh Gaussian initialization noise must have shape "
                    f"{expected_shape}, got {tuple(noise.shape)}."
                )
            return noise, timestep_grid
    ...
    def _forward_denoiser(
        ...
        *,
        ...
        if self.anchor_encoder is not None:
            anchors = self.expand_anchors(
                batch_size,
                device=candidates.device,
                dtype=candidates.dtype,
            )
            anchor_features = self.anchor_encoder(
                anchors.reshape(batch_size * self.num_anchors, self.action_chunk_dim)
            ).reshape(batch_size, self.num_anchors, -1)
            candidate_features = candidate_features + anchor_features
    ...
    def denoise_candidate_step(
        ...
        *,
        ...
        """Dispatch one reverse step to the configured diffusion process."""
        if self.diffusion_mode == "bridge":
            if float(eta) != 0.0:
                raise ValueError("eta is not supported by bridge diffusion sampling.")
            anchors = self.expand_anchors(
                int(current.shape[0]),
                device=current.device,
                dtype=current.dtype,
            )
            return denoise_step_from_bridge_x0(
                current,
                x0_pred,
                anchors,
                timesteps=timesteps,
        ...
        return denoise_step_from_x0(
            current,
            x0_pred,
            timesteps=timesteps,
            next_timesteps=next_timesteps,
            schedule=self.schedule.to(device=current.device, dtype=current.dtype),
            eta=eta,
            noise=noise,
            action_chunk_dim=self.action_chunk_dim,
        )
    ...
    def generate_candidates(
        ...
        *,
        ...
        """Run the configured reverse process from anchors to final candidate chunks.
        ...
        if self.diffusion_mode == "bridge" and float(eta) != 0.0:
            raise ValueError("eta is not supported by bridge diffusion sampling.")
        ...
        current, _ = self.initialize_noisy_candidates(
            ...
            noise=None if self.diffusion_mode == "bridge" else initial_noise,
        )  # [B, K, action_chunk_dim]
        initial_noisy = current.clone()

        intermediates: list[torch.Tensor] = []
        last_outputs: dict[str, torch.Tensor] | None = None
        step_pairs = self.build_inference_step_pairs(runtime_truncation_timesteps)
        ...
        for current_t, next_t in step_pairs:
            ...
            reverse_noise = None
            if self.diffusion_mode == "bridge" and current_t == self.bridge_schedule.terminal_timestep:
                reverse_noise = initial_noise
            elif eta > 0.0:
                reverse_noise = torch.randn_like(current) * float(sampling_temperature)
            current = self.denoise_candidate_step(
                current,
                x0_pred,
                timesteps=timestep_grid,
...
class DiffusionPlannerBundle:
    """Portable save/load bundle for the anchor-conditioned diffusion planner."""
