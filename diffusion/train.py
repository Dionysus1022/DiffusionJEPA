...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--initialization-mode",
        choices=["anchors", "gaussian"],
        default="anchors",
        help=(
            "Candidate initialization. `anchors` adds schedule noise around the "
            "stored anchor values; `gaussian` samples fresh [B,K,D] standard noise "
            "and uses the anchor bundle only for K and D."
        ),
    )
    parser.add_argument(
        "--diffusion-mode",
        choices=["truncated", "bridge"],
        default="truncated",
        help="Diffusion process: legacy noisy-anchor truncation or a symmetric anchor-to-plan bridge.",
    )
    parser.add_argument(
        "--bridge-beta-d",
        type=float,
        default=2.0,
        help="Quadratic VP rate used by bridge diffusion.",
    )
    parser.add_argument(
        "--bridge-beta-min",
        type=float,
        default=0.1,
        help="Minimum VP rate used by bridge diffusion.",
    )
    parser.add_argument(
        "--truncation-steps",
        type=int,
...
def build_model_from_dataset_and_anchor_bundle(
    ...
    model_cfg = infer_model_config_from_dataset_and_anchor_bundle(
        ...
        bridge_beta_d=args.bridge_beta_d,
        bridge_beta_min=args.bridge_beta_min,
    )
...
def sample_timestep_grid(
    ...
    *,
    ...
    if sampling == "truncation":
        choices = model.resolve_inference_truncation_timesteps(device=device)
        if model.diffusion_mode == "bridge":
            choices = choices[:-1]  # The final state node is x_0, not a denoiser input.
    ...
    if sampling == "full":
        timestep_per_sample = torch.randint(
            low=1 if model.diffusion_mode == "bridge" else 0,
...
def build_eval_timestep_grid(
    ...
    *,
    ...
    timestep = int(model.resolve_inference_truncation_timesteps(device=device)[0].item())
    return torch.full(
...
def assign_positive_anchors(
    ...
    anchors: [K, action_chunk_dim] or batch-specific [B, K, action_chunk_dim]
    ...
    if anchors.ndim == 2:
        candidate_references = anchors.unsqueeze(0)
    elif anchors.ndim == 3:
        if int(anchors.shape[0]) != int(teacher_plan.shape[0]):
            raise ValueError(
                "Batch-specific candidate references must match teacher_plan batch size: "
                f"{anchors.shape[0]} != {teacher_plan.shape[0]}."
            )
        candidate_references = anchors
    else:
        raise ValueError(
            "anchors must have shape [K, D] or [B, K, D], "
            f"got {tuple(anchors.shape)}."
        )
    if int(teacher_plan.shape[-1]) != int(candidate_references.shape[-1]):
        raise ValueError(
            "teacher_plan dim does not match candidate-reference dim: "
            f"{teacher_plan.shape[-1]} != {candidate_references.shape[-1]}."
        )

    squared_distances = (
        teacher_plan.unsqueeze(1) - candidate_references
...
def forward_inference_candidate_scores(
    ...
    *,
    ...
    """Run the configured inference denoising loop and return final candidates/scores.
    ...
    current, _ = model.initialize_noisy_candidates(
        ...
        noise=None if model.diffusion_mode == "bridge" else noise,
    )

    step_pairs = model.build_inference_step_pairs(runtime_truncation_timesteps)
    ...
    for current_t, next_t in step_pairs:
        ...
        reverse_noise = None
        if model.diffusion_mode == "bridge" and current_t == model.bridge_schedule.terminal_timestep:
            reverse_noise = noise
        elif eta > 0.0:
            reverse_noise = torch.randn_like(current)
        current = model.denoise_candidate_step(
            current,
            x0_pred,
...
def compute_batch_losses(
    *,
    ...
    noisy_candidates, _ = model.initialize_noisy_candidates(
        ...
        clean_candidates=teacher_plan if model.diffusion_mode == "bridge" else None,
    ...
    assignment_references = (
        noisy_candidates.detach()
        if model.initialization_mode == "gaussian"
        else model.anchors.to(device=device, dtype=teacher_plan.dtype)
    )
...
def evaluate(
    ...
    validation_generator = torch.Generator(device=device).manual_seed(int(args.seed) + 100_003)
    for batch in loader:
        batch_size = int(batch["z_cur"].shape[0])
...
def main(argv: list[str] | None = None) -> None:
    ...
    print(
        ...
        f"initialization_mode={model.initialization_mode} "
        f"diffusion_mode={model.diffusion_mode} "
        f"bridge_beta_d={model.config.bridge_beta_d} "
        f"bridge_beta_min={model.config.bridge_beta_min} "
