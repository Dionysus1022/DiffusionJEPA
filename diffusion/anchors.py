...
class ActionAnchorBundle:
    ...
    fit_method: str = "kmeans_nearest_real_sample"
    seed: int | None = None
...
def select_nearest_real_anchors(
    teacher_plan: torch.Tensor,
    centroids: torch.Tensor,
    labels: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, int]:
    """Replace each centroid with the nearest real chunk assigned to its cluster."""
    teacher_plan = torch.as_tensor(teacher_plan).detach().cpu().float()
    centroids = torch.as_tensor(centroids).detach().cpu().float()
    labels = torch.as_tensor(labels, dtype=torch.long).detach().cpu().reshape(-1)
    if teacher_plan.ndim != 2 or centroids.ndim != 2:
        raise ValueError("teacher_plan and centroids must both be rank-2 tensors.")
    if teacher_plan.shape[0] != labels.numel():
        raise ValueError("labels must contain one cluster id per teacher_plan row.")
    if teacher_plan.shape[1] != centroids.shape[1]:
        raise ValueError("teacher_plan and centroids must have the same feature width.")
    ...
    for cluster_index in range(int(centroids.shape[0])):
        candidate_indices = torch.nonzero(labels == cluster_index, as_tuple=False).flatten()
        if candidate_indices.numel() == 0:
            empty_cluster_count += 1
            candidate_indices = torch.arange(teacher_plan.shape[0], dtype=torch.long)

        candidate_rows = teacher_plan.index_select(0, candidate_indices)
        distances = torch.linalg.vector_norm(
            candidate_rows - centroids[cluster_index].unsqueeze(0),
            ord=2,
            dim=1,
        )
        ordered_positions = torch.argsort(distances)
        chosen_index = int(candidate_indices[int(ordered_positions[0])].item())
        for position in ordered_positions.tolist():
            proposed_index = int(candidate_indices[int(position)].item())
            if proposed_index not in used_indices:
                chosen_index = proposed_index
                break
        used_indices.add(chosen_index)

        selected_row = teacher_plan[chosen_index]
        selected_rows.append(selected_row)
        selected_indices.append(chosen_index)
        selected_distances.append(
            float(
                torch.linalg.vector_norm(
                    selected_row - centroids[cluster_index], ord=2
                ).item()
            )
        )
...
def fit_action_anchors(
    ...
    *,
    ...
    """Fit K-means clusters and use the nearest real action chunk as each anchor.
    ...
    labels = kmeans.fit_predict(teacher_plan_tensor.numpy())
    centroids = torch.from_numpy(kmeans.cluster_centers_).float()
    anchors, selected_indices, centroid_distances, empty_cluster_count = (
        select_nearest_real_anchors(
            teacher_plan_tensor,
            centroids,
    ...
    bundle_metadata.update(
        {
            ...
            "selected_teacher_plan_indices": selected_indices.tolist(),
            "centroid_to_real_l2_mean": float(centroid_distances.mean().item()),
            "centroid_to_real_l2_max": float(centroid_distances.max().item()),
